...
function Pager({ page, pages, total, perPage, onPage }: { page: number; pages: number; total: number; perPage: number; onPage: (p: number) => void }) {
  const t = useT();
  if (pages <= 1) return <p className="mt-3 text-center text-xs text-muted">{n(total)} {t('partiya')}</p>;
  const from = (page - 1) * perPage + 1;
  const to = Math.min(page * perPage, total);
  const btn = 'grid h-8 min-w-8 place-items-center rounded-lg border px-2 text-sm transition';
  return (
    <nav className="mt-3 flex flex-col items-center justify-between gap-2 sm:flex-row" aria-label={t('Sahifalash')}>
      <p className="text-xs tabular-nums text-muted">
        <span className="font-semibold text-fg">{n(from)}</span>–<span className="font-semibold text-fg">{n(to)}</span> / {n(total)} {t('partiya')}
      </p>
      <div className="flex items-center gap-1">
        <button onClick={() => onPage(page - 1)} disabled={page <= 1} aria-label={t('Oldingi')} className={`${btn} border-line ${page <= 1 ? 'pointer-events-none opacity-40' : 'hover:bg-surface-2'}`}>‹</button>
        ...
        <button onClick={() => onPage(page + 1)} disabled={page >= pages} aria-label={t('Keyingi')} className={`${btn} border-line ${page >= pages ? 'pointer-events-none opacity-40' : 'hover:bg-surface-2'}`}>›</button>
      </div>
    </nav>
  );
}
  ...
  return (
    <div className="card p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          <div className="text-sm font-semibold">{t('Partiyalar tarixi')}</div>
          <div className="mt-0.5 text-xs text-muted">{t('So‘nggi orqa-fon partiyalari (talabnoma PDF, ariza, oferta, import) — holati va yuklab olish.')}</div>
        </div>
        <button onClick={load} className="rounded-lg border border-line px-2.5 py-1 text-xs font-medium text-muted transition-colors hover:border-brand-500/40">{t('Yangilash')}</button>
      </div>
      ...
      {err && !rows ? (
        ...
        <div className="grid h-16 place-items-center text-center text-xs text-muted">{t('Hali partiya yo‘q.')}</div>
      ) : (
        <>
          <div className="overflow-x-auto rounded-lg border border-line">
            <table className="w-full text-sm">
              <thead className="border-b border-line bg-surface text-left text-xs text-muted">
                <tr>
                  <th className="px-3 py-2 font-medium">{t('Sana')}</th>
                  <th className="px-3 py-2 font-medium">{t('Tur')}</th>
                  <th className="px-3 py-2 font-medium">{t('Firma')}</th>
                  <th className="px-3 py-2 font-medium">{t('Soni')}</th>
                  <th className="px-3 py-2 font-medium">{t('Holat')}</th>
                ...
                {pageRows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-line">
                      ...
                      <td className="px-3 py-2 font-medium">{t(TYPE_LABEL(r))}</td>
                      <td className="px-3 py-2 text-xs text-muted">{r.firmName}</td>
                      <td className="px-3 py-2 text-xs tabular-nums">
                        {r.status === 'RUNNING' ? `${n(r.progress)}/${n(r.total)} · ${pct}%` : n(r.total)}
                      </td>
                      <td className="px-3 py-2"><span className={`rounded-md px-1.5 py-0.5 text-[11px] font-medium ${st.tone}`}>{t(st.label)}</span></td>
                      <td className="px-3 py-2 text-right">
                        {r.status === 'DONE' && r.hasResult
                          ? <a href={`/api/export/${r.id}/download`} className="inline-flex items-center gap-1 rounded-md border border-line px-2 py-0.5 text-[11px] font-medium text-brand-600 hover:border-brand-500/40 dark:text-brand-400"><Ico.download size={13} /> {t('Yuklab olish')}</a>
                          : r.status === 'FAILED' && r.message
                            ? <span className="text-[11px] text-rose-500" title={r.message}>{t('xato')}</span>
                            : null}
                      </td>
                    </tr>
                  );
