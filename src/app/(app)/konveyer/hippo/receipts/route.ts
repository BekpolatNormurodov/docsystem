...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (wanted.length === 0) return NextResponse.json({ error: t('Yetkazilgan kvitansiya topilmadi') }, { status: 404 });
    ...
    if (ok === 0) return NextResponse.json({ error: t('Kvitansiyalar yuklab bo‘lmadi') }, { status: 502 });
    ...
    return NextResponse.json({ error: t('Kvitansiya yuklab bo‘lmadi') }, { status: 502 });
  }
}
