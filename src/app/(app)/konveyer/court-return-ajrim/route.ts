...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (e instanceof SessionExpiredError)
      return NextResponse.json({ error: t('Cabinet kaliti ulanmagan — «Ulanishlar» orqali E-IMZO bilan qayta ulang.'), needAuth: true }, { status: 401 });
    console.error('court-return-ajrim failed', e);
    return NextResponse.json({ error: t('Ajrim maʼlumoti olinmadi') }, { status: 500 });
  }
}
