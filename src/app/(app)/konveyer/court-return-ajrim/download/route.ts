...
export async function GET(req: NextRequest) {
  ...
  if (!caseNumber) return NextResponse.json({ error: t('caseNumber kerak') }, { status: 400 });
  try {
    const r = await downloadReturnAjrim(caseNumber);
    if (!r) return NextResponse.json({ error: t('Ajrim topilmadi') }, { status: 404 });
    ...
    if (e instanceof SessionExpiredError)
      return NextResponse.json({ error: t('Cabinet kaliti ulanmagan — E-IMZO bilan qayta ulang.'), needAuth: true }, { status: 401 });
    console.error('court-return-ajrim download failed', e);
    return NextResponse.json({ error: t('Ajrim yuklab boʻlmadi') }, { status: 500 });
  }
}
