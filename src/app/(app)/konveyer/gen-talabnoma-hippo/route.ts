...
export async function GET(req: NextRequest) {
  ...
  if (!st?.caseNumber) return NextResponse.json({ error: t('xat.hippo da yetkazilgan talabnoma topilmadi (avval joʻnatish/ingest kerak)') }, { status: 404 });
  ...
  try {
    ...
    return NextResponse.json({ error: t('Yetkazilgan talabnoma yuklab boʻlmadi') }, { status: 502 });
  }
}
