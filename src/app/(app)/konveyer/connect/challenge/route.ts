...
export async function POST(req: NextRequest) {
  ...
  if (!account) return NextResponse.json({ error: t('Firmada STIR yoʻq — avval STIR kiriting') }, { status: 400 });
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : t('Challenge olinmadi');
    console.error(`cabinet challenge firm ${firmId} failed:`, msg);
    return NextResponse.json({ error: `${t('Challenge olinmadi')}: ${msg.slice(0, 200)}` }, { status: 502 });
  }
}
