...
export async function POST(req: NextRequest) {
  ...
  if (!firmId || (provider !== 'HIPPO' && provider !== 'CABINET')) {
    return NextResponse.json({ error: t('firmId va provider (HIPPO|CABINET) kerak') }, { status: 400 });
  }
  ...
  if (!account) {
    return NextResponse.json({ error: t('Firmada STIR yoʻq — ulanishdan oldin STIR kiriting') }, { status: 400 });
  }
  ...
  if (eimzoMode() === 'client' && pkcs7) {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : t('E-IMZO ulanmadi');
      console.error(`connect(client) ${provider} firm ${firmId} failed:`, msg);
      return NextResponse.json({ error: `${t('Kalit ulanmadi')}: ${msg.slice(0, 200)}`, needsKey: true }, { status: 502 });
    }
  }
  ...
  try {
    ...
    return NextResponse.json({ error: `${t('Kalit ulanmadi')}: ${msg.slice(0, 160)}`, needsKey: true }, { status: 502 });
  }
}
