...
export async function GET(req: NextRequest) {
  ...
  if (usable.length === 0) return NextResponse.json({ error: t('Grafik uchun maʼlumot yetarli emas (summa/muddat yoʻq)') }, { status: 404 });
  ...
  try {
    ...
    return NextResponse.json({ error: t('Grafik yaratilmadi') }, { status: 500 });
  }
