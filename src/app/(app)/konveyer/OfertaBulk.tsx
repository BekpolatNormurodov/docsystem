...
export function OfertaBulk({ firms, snapshotId }: { firms: Firm[]; snapshotId?: number }) {
  ...
  const scopeLabel = firm ? firm.firmName : t('Hamma firma');
    ...
    <div className="space-y-4">
      ...
      <div className="card p-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div>
            <div className="text-sm font-semibold">{t('Oferta tayyorlash')}</div>
            <div className="mt-0.5 text-xs text-muted">{t('Har shartnomaga oferta (mikroqarz shartnomasi) PDF → bitta ZIP (mijoz papkalari) ·')} {scopeLabel}</div>
          </div>
          {countBusy && totalAll == null
            ...
            : totalAll != null && (
              <span className="shrink-0 rounded-lg bg-surface-2 px-2 py-1 text-xs font-semibold tabular-nums">
                {n(totalAll)} {t('mijoz')}{doneCount > 0 && <span className="ml-1 font-normal text-emerald-600 dark:text-emerald-400">· {n(count ?? 0)} {t('qoldi')}</span>}
              </span>
            )}
        </div>
              ...
              <div className="mb-3 flex flex-wrap gap-1.5">
                {firms.map((f) => (
                  <button key={f.firmId} onClick={() => setFirmId(f.firmId)} disabled={!!running || starting}
                    ...
                    title={`«${f.firmName}» ${t('boʻyicha oferta tayyorlash')}`}>
        ...
        {running ? (
          <div className="rounded-xl border border-brand-500/25 bg-brand-500/[0.04] p-3">
            <div className="flex items-center justify-between gap-2">
              <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-700 dark:text-brand-300">
                <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-current border-t-transparent" /> {t('Yaratilmoqda…')}
              </span>
            ...
            <div className="mt-1 flex flex-wrap items-center justify-between gap-x-3 text-xs text-muted tabular-nums">
              <span>{n(job?.progress ?? 0)} / {n(job?.total ?? 0)} {t('mijoz')}</span>
            ...
            <div className="mt-2.5 flex justify-end">
              <button onClick={cancel} disabled={canceling} aria-busy={canceling}
                ...
                {canceling
                  ...
                  : <><Ico.close size={13} /> {t('Bekor qilish')}</>}
              </button>
            </div>
          </div>
        ) : canceled ? (
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-rose-500/30 bg-rose-500/[0.06] px-3 py-1.5 text-xs font-semibold text-rose-600 dark:text-rose-300">
              <Ico.close size={14} /> {t('Bekor qilindi — hammasi oʻchirildi')}
            </span>
            <button onClick={() => { setJobId(null); setJob(null); loadCount(); }} className="rounded-lg border border-line px-2.5 py-1.5 text-xs font-medium text-muted transition-colors hover:bg-surface-2">{t('Qaytadan')}</button>
          </div>
        ) : done ? (
          <div className="flex flex-wrap items-center gap-2">
            <a href={`/api/export/${jobId}/download`}
              ...
              <Ico.download size={14} /> {job?.message || `${n(job?.total ?? 0)} ${t('tayyor')}`} — {t('yuklab olish')}
            </a>
          ...
          <div className="flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/[0.07] px-3 py-2 text-xs font-medium text-emerald-700 dark:text-emerald-300">
            ...
            <span>{t('Hammasi tayyor —')} {n(doneCount)} {t('ta mijozga oferta chiqarilgan.')}</span>
          </div>
        ) : (
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              title={`«${scopeLabel}» ${t('boʻyicha ofertalarni yaratish')}`}
            >
              {starting ? <><span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" /> {t('Boshlanmoqda…')}</> : <><Ico.flash size={14} /> {t('Oferta yaratish')}{count != null ? ` (${n(total)})` : ''}</>}
            </button>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={t('Oferta tayyorlash')}>
          ...
          <div className="animate-fade-in relative w-full max-w-sm rounded-2xl border border-line bg-surface p-5 shadow-2xl">
            ...
            <div className="mb-1 flex items-center gap-2.5">
              ...
              <div className="text-base font-semibold">{t('Oferta tayyorlash')}</div>
            </div>
            <div className="mb-4 text-xs text-muted">«{scopeLabel}» · {t('og‘ir jarayon — orqada yaratiladi, tayyor bo‘lgach ZIP yuklab olasiz.')}</div>
            ...
            <div className="mt-3 flex items-start gap-1.5 rounded-lg border border-line bg-surface-2/40 px-3 py-2 text-[11px] leading-relaxed text-muted">
              ...
              <span>{t('Sugʻurta (taʼminot) summasi')} <b className="font-medium text-fg">{t('portfeldan')}</b> {t('— har shartnomaning oʻz maʼlumotidan (sugʻurta summasining 4%) avtomatik olinadi. Qoʻlda kiritilmaydi.')}</span>
            </div>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              <button onClick={start} disabled={starting || !valid || total === 0} aria-busy={starting}
                ...
                {starting ? <><span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" /> {t('Boshlanmoqda…')}</> : <><Ico.flash size={14} /> {t('Yaratish')} ({n(batchN)})</>}
              </button>
            </div>
          </div>
        </div>
