...
export function CourtDetail({ caseId }: { caseId: number }) {
  ...
  useEffect(() => {
    ...
    fetch(`/konveyer/court-case?caseId=${caseId}`)
      ...
      .catch(() => { if (my === reqRef.current) setErr(t('Sud maʼlumotini yuklab boʻlmadi')); })
  ...
  if (!data.found) {
    return (
      <div className="rounded-lg border border-dashed border-line bg-surface-2/40 px-3 py-3 text-center text-[11px] text-muted">
        {t('Sud (cabinet.sud.uz) maʼlumoti hali olinmagan.')}
        {data.caseNumber && <span className="ml-1">{t('Ish raqami:')} <b className="font-mono text-fg">{data.caseNumber}</b>.</span>}
        <div className="mt-0.5">{t('«Ulanishlar» dan cabinet.sud.uz ga ulanib statuslarni sinxronlang.')}</div>
      </div>
    );
  }
    ...
    <div className="rounded-xl border border-line bg-surface p-3">
      ...
      {badge?.bad && (
        <div className="mb-2 rounded-lg border border-rose-500/25 bg-rose-500/[0.05] px-3 py-2 text-[11px] leading-relaxed text-rose-700 dark:text-rose-300">
          <b className="font-semibold">{t('Ariza suddan qaytarilgan.')}</b> {t('Kamchilik/sabab matni cabinet.sud.uz dagi')} <b>{t('ajrim')}</b>{t("da koʻrsatiladi — bu matn cabinet API'sida yoʻq (faqat saytda ochib koʻriladi). Tuzatib qayta topshiriladi.")}
          {dmy(data.definitionDate) && <span> · {t('Ajrim sanasi:')} <b className="font-mono">{dmy(data.definitionDate)}</b></span>}
          <a href="https://cabinet.sud.uz" target="_blank" rel="noreferrer" className="ml-1 inline-flex items-center gap-0.5 font-semibold underline decoration-rose-400/50 underline-offset-2 hover:decoration-rose-400">{t('cabinet.sud.uz da ochish ↗')}</a>
        </div>
      )}
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3">
        <Field label={t('Ish raqami')} value={data.caseNumber} mono />
        <Field label={t('Reyestr raqami')} value={data.registryNumber} mono />
        <Field label={t('Tur')} value={data.claimKind ? t(KIND_UZ[data.claimKind] ?? data.claimKind) : null} />
        <Field label={t('Bosqich')} value={data.instance} />
        <Field label={t('Sudya')} value={data.judge} pending={t('detailda yoʻq')} />
        <Field label={t('Majlis sanasi')} value={dmy(data.hearingDate)} />
        <Field label={t("Ro'yxatga olingan")} value={dmy(data.registryDt)} />
        <Field label={t('Ajrim sanasi')} value={dmy(data.definitionDate)} />
        <Field label={t('Javobgar manzili')} value={data.defAddress} />
        <Field label={t('Modda')} value={data.modda} pending={t('sud hujjatida')} />
        <Field label={t('Ijrochi (davlat)')} value={data.ijrochi} pending={t('MIB API ulangach')} />
        <Field label={t('Oylikka qaratilgan')} value={data.oylik} pending={t('MIB API ulangach')} />
      </div>
      ...
      {(docsLoading || ajrimlar.length > 0 || otherDocs.length > 0) && (
        <div className="mt-3 border-t border-line pt-2">
          <div className="mb-1.5 text-[10px] font-medium uppercase tracking-wide text-muted">{t('Sud hujjatlari')}</div>
          {docsLoading ? (
