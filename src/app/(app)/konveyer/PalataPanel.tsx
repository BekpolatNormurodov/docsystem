...
export function PalataPanel() {
  ...
  const doAssign = async (caseId: number) => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); throw new Error(d?.error ?? t('Biriktirib bo‘lmadi')); }
      setAssign(null); setQ(''); setResults([]);
      await load();
    } catch (e) {
      setErr(e instanceof Error ? e.message : t('Biriktirib bo‘lmadi'));
  ...
  const onFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('Yuklab bo‘lmadi'));
      await load();
    } catch (e2) {
      setErr(e2 instanceof Error ? e2.message : t('Yuklab bo‘lmadi'));
    } finally {
  ...
  const del = async (name: string) => {
    try {
      ...
      if (!res.ok) throw new Error(t('O‘chirib bo‘lmadi'));
      setFiles((f) => f.filter((x) => x.name !== name));
    } catch (e) { setErr(e instanceof Error ? e.message : t('O‘chirib bo‘lmadi')); }
  };

  return (
    <div className="card p-4">
      <div className="mb-3">
        <div className="text-sm font-semibold">{t('Palatadan kelgan imzolangan arizalar')}</div>
        <div className="mt-0.5 text-xs text-muted">{t('Uzun skanni yoki bittalab yuklang — keyin arizalarga ajratiladi.')}</div>
      </div>

      <div
        role="button"
        tabIndex={0}
        aria-label={t('Skanerlangan PDF yuklash')}
        aria-busy={busy}
      ...
      >
        ...
        <div className="text-sm font-medium">{busy ? t('Yuklanmoqda…') : t('Skanerlangan PDF(lar)ni yuklang')}</div>
        <div className="text-[11px] text-muted">{t('bosing yoki tashlang · PDF · bir nechta')}</div>
      ...
      {err && (
        <div role="alert" className="mt-2 flex items-center justify-between gap-2 rounded-lg border border-rose-500/25 bg-rose-500/[0.04] px-2.5 py-1.5 text-[11px] font-medium text-rose-500">
          <span>{err}</span>
          <button onClick={() => { setLoading(true); load(); }} className="shrink-0 rounded border border-line px-1.5 py-0.5 text-muted hover:border-brand-500/40">{t('Qayta urinish')}</button>
        </div>
      )}
      ...
      : files.length === 0 ? (
        <div className="mt-3 text-center text-[11px] text-muted">{t('Hali skan yuklanmagan.')}</div>
      ) : (
        <div className="mt-3">
          <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted">{t('Tarix — yuklangan skanlar')} · {files.length}</div>
            ...
            {files.map((f) => (
              <li key={f.name} className="rounded-lg border border-line bg-surface">
                <div className="flex items-center gap-2 px-2.5 py-1.5 text-xs">
                  ...
                  <button onClick={() => { setAssign(assign === f.name ? null : f.name); setQ(''); }} aria-expanded={assign === f.name} className="shrink-0 rounded border border-line px-1.5 py-0.5 text-[11px] font-medium text-brand-600 hover:border-brand-500/40 dark:text-brand-400">→ {t('Arizaga')}</button>
                  <button onClick={() => del(f.name)} className="shrink-0 text-rose-500 hover:text-rose-600" aria-label={t("O'chirish")}>✕</button>
                </div>
                {assign === f.name && (
                  <div className="border-t border-line p-2">
                    <input value={q} onChange={(e) => setQ(e.target.value)} autoFocus aria-label={t('Mijoz qidirish')} placeholder={t('Mijoz qidirish (F.I.O, PINFL)…')} className="mb-1.5 w-full rounded-lg border border-line bg-surface px-2.5 py-1.5 text-xs outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/15" />
                    {searching ? (
                      <div className="flex items-center justify-center gap-2 py-2 text-[11px] text-muted"><span className="h-3 w-3 animate-spin rounded-full border-2 border-muted/40 border-t-muted" /> {t('Qidirilmoqda…')}</div>
                    ) : results.length === 0 ? (
                      <div className="py-2 text-center text-[11px] text-muted">{q ? t('Topilmadi') : t('Qidiring…')}</div>
                    ) : (
          ...
          <div className="mt-2 text-[11px] text-muted">{t('Bittalab biriktirish pastdagi mijoz kartasidan ham mumkin.')}</div>
        </div>
      )}
    </div>
  );
