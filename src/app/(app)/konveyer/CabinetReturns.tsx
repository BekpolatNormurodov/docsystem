  ...
  useEffect(() => {
    ...
    fetch(`/konveyer/court-return-ajrim?caseNumber=${encodeURIComponent(r.caseNumber)}`)
      .then(async (res) => {
        ...
        if (!res.ok) { setAjrimErr({ msg: d?.error || t('Ajrim olinmadi'), needAuth: !!d?.needAuth }); return; }
        setAjrim(d);
      })
      .catch(() => setAjrimErr({ msg: t('Tarmoq xatosi'), needAuth: false }))
        ...
        <div className="space-y-3 border-t border-line bg-surface-2/30 p-3">
          ...
          <div className="rounded-lg border border-brand-500/25 bg-brand-500/[0.04] px-3 py-2.5">
            <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted">
              <Ico.judge size={14} className="text-brand-500" /> {t('Sud ajrimi (cabinet.sud.uz)')}
            </div>
            {ajrimLoading ? (
              <div className="flex items-center gap-2 text-[12px] text-muted"><span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-brand-500/40 border-t-brand-500" /> {t('Ajrim olinmoqda…')}</div>
            ) : ajrimErr ? (
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-[12px] text-muted">
                  {ajrim.judge && <span>{t('Sudya:')} <b className="font-medium text-fg">{ajrim.judge}</b></span>}
                ...
                {ajrim.available ? (
                  ...
                  <div className="text-[12px] text-muted">{t('Ajrim PDF hali mavjud emas.')}</div>
                )}
                <div className="text-[11px] text-muted">{t('Kamchilikning aniq matni shu ajrim ichida yozilgan.')}</div>
              </div>
            ) : (
              <div className="text-[12px] text-muted">{t('Ajrim maʼlumoti yoʻq.')}</div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Meta label={t('Ish raqami')} value={r.caseNumber} mono />
            <Meta label={t('Registratsiya №')} value={r.registryNumber} mono />
            <Meta label={t('Ajrim sanasi')} value={dmy(r.definitionDate)} />
            <Meta label={t('Ro‘yxatga olingan')} value={dmy(r.registryDt)} />
          </div>

          <div className="flex items-start gap-1.5 text-[11px] leading-relaxed text-muted">
            <Ico.info size={13} className="mt-0.5 shrink-0" />
            <span>{t('Aniq izoh (kamchilik matni) —')} <b className="font-medium text-fg">cabinet.sud.uz</b> {t('dagi ajrimda. Ish raqami bo‘yicha «Mening ishlarim» → ajrimni oching. Tuzatib qayta topshirish ham o‘sha yerda.')}</span>
          </div>
        </div>
      )}
    </div>
...
export function CabinetReturns({ snapshotId, firmId }: { snapshotId?: number; firmId?: number }) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (e) { if (my === reqRef.current) setErr(e instanceof Error ? e.message : t('Yuklanmadi')); }
  ...
  return (
    <div className="card p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-sm font-semibold">{t('Suddan qaytgan ishlar (cabinet.sud.uz)')}</div>
          <div className="mt-0.5 text-xs text-muted">{t('Sud arizani qaytargan/rad etgan mijozlar — har birini ochib sababi va tavsiyani ko‘ring, tuzatib qayta topshiring.')}</div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={load} disabled={loading} aria-label={t('Yangilash')} title={t('Yangilash')} className="grid h-9 w-9 place-items-center rounded-xl border border-line text-muted outline-none transition-colors hover:border-brand-500/40 hover:text-fg focus-visible:ring-2 focus-visible:ring-brand-500/30 disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="grid h-16 place-items-center text-center text-xs text-muted">{t('Qaytgan ish yoʻq.')}</div>
      ) : (
        <>
          ...
          <div className="mb-2 flex flex-wrap items-center gap-1.5">
            ...
            >
              {t('Hammasi')} <span className="tabular-nums">{n(data.total)}</span>
            </button>
            {codes.map((code) => {
            ...
            {resultF !== 'all' && (
              <button onClick={() => setResultF('all')} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[11px] font-medium text-muted transition-colors hover:text-fg" title={t('Filterni tozalash')}>
          ...
          <div className="relative mb-2">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} aria-label={t('Qidirish')} placeholder={t('F.I.O, PINFL, firma yoki ish raqami…')} className="w-full rounded-xl border border-line bg-surface py-2 pl-10 pr-3 text-sm outline-none transition-colors focus:border-brand-500 focus:ring-2 focus:ring-brand-500/15" />
          </div>
          ...
          <div className="mt-2 text-[11px] text-muted">{t('Har satrni ochsangiz — cabinet.sud.uz dan')} <b className="font-medium text-fg">{t('sud ajrimi')}</b> {t('(turi, sudya, sud) va')} <b className="font-medium text-fg">{t('ajrim PDF')}</b> {t('(aniq sabab), hamda natija ma’nosi va tavsiya ko‘rinadi.')}</div>
        </>
      )}
    </div>
  );
