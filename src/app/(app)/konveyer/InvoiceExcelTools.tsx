...
export function InvoiceExcelTools({ snapshotId, firmId, firms, count, onChanged, showExport = true }: {
  ...
  const send = async (f: File, apply: boolean): Promise<ImportResult> => {
    ...
    if (!res.ok) throw new Error(d?.error ?? t('Import xatosi'));
  ...
  const pickFile = async (f?: File | null) => {
    if (!f) return;
    if (gateFile) { setErr(t('Avval firmani tanlang')); return; }
    if (!/\.xlsx$/i.test(f.name)) { setErr(t('Faqat .xlsx fayl')); return; }
    ...
    catch (e) { setErr(e instanceof Error ? e.message : t('Import xatosi')); }
  ...
  const impFooter = done ? (
    <button onClick={() => setImpOpen(false)} className="btn-primary px-3 py-2 text-sm">{t('Yopish')}</button>
  ) : preview ? (
    <>
      <button onClick={resetPick} disabled={busy} className="btn-ghost px-3 py-2 text-sm">{t('Boshqa fayl')}</button>
      <button onClick={confirmApply} disabled={busy || preview.willAssign === 0} className="btn-primary gap-1.5 px-3 py-2 text-sm">
        {busy && <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" />} {t('Bazaga saqlash')}{preview.willAssign ? ` (${n(preview.willAssign)})` : ''}
      </button>
    </>
  ) : (
    <button onClick={() => setImpOpen(false)} className="btn-ghost px-3 py-2 text-sm">{t('Yopish')}</button>
  );
    ...
    <div className="flex flex-wrap items-center gap-2">
      {showExport && (
        <button onClick={openExport}
          ...
          <Ico.sheet size={13} /> {t('Excel eksport')}{typeof count === 'number' ? ` (${n(count)})` : ''}
        </button>
      )}
      <button onClick={openImport}
        ...
        <Ico.download size={13} /> {t('Excel import (kvitansiya)')}
      </button>
      {done && !impOpen && <span role="status" className="text-[11px] font-medium text-emerald-600 dark:text-emerald-400">✓ {n(done.assigned)} {t('ta invoice chiqarildi')}</span>}

      {/* ── EXPORT MODAL ── */}
      <Modal open={expOpen} onClose={() => setExpOpen(false)} size="md"
        title={t('Excel eksport — invoyslar')}
        description={t('Firma va holatni tanlang — roʻyxat «Holat» ustuni bilan yuklanadi')}
        footer={
          <>
            <button onClick={() => setExpOpen(false)} className="btn-ghost px-3 py-2 text-sm">{t('Bekor')}</button>
            <a href={exportUrl} onClick={() => setExpOpen(false)}
              ...
              <Ico.download size={15} /> {t('Excel yuklab olish')}{expCount ? ` (${n(expCount)})` : ''}
            </a>
          </>
        }>
        <div className="space-y-4">
          {needFirmPick && (
            <div>
              <div className="field-label">{t('Firma')}</div>
              <Select value={expFirm ? String(expFirm) : ''} label={t('Eksport firmasi')} placeholder={t('Barcha firmalar')}
                options={[{ value: '', label: t('Barcha firmalar') }, ...firms!.map((f) => ({ value: String(f.id), label: f.name }))]}
                onChange={(v) => setExpFirm(v ? Number(v) : '')} />
            </div>
          )}
          {!needFirmPick && firmNameOf(firmId) && (
            <div><div className="field-label">{t('Firma')}</div><div className="rounded-xl border border-line bg-surface-2/40 px-3 py-2 text-sm font-medium">{firmNameOf(firmId)}</div></div>
          )}
          <div>
            <div className="field-label">{t('Holat')}</div>
            <div className="grid grid-cols-3 gap-2">
              {FILTERS.map(([v, l, hint]) => (
                <button key={v} type="button" onClick={() => setExpFilter(v)} aria-pressed={expFilter === v}
                  ...
                  <div className={`text-xs font-semibold ${expFilter === v ? 'text-brand-700 dark:text-brand-300' : 'text-fg'}`}>{t(l)}</div>
                  <div className="mt-0.5 text-[10px] leading-tight text-muted">{t(hint)}</div>
                </button>
              ))}
            </div>
          </div>
          <div className="flex items-center justify-between rounded-xl border border-line bg-surface-2/40 px-3 py-2.5 text-sm">
            <span className="text-muted">{t('Yuklanadi:')}</span>
            <span className="font-bold tabular-nums text-brand-700 dark:text-brand-300">{expLoading ? '…' : expCount != null ? `${n(expCount)} ${t('ta')}` : '—'}</span>
          </div>
          <div className="text-[11px] leading-relaxed text-muted">{t('Ustunlar:')} F.I.O · PINFL · {t('Firma')} · {t('Sud')} · {t('Kvitansiya raqami')} · <b className="text-fg">{t('Holat')}</b> · {t('Sana')}.</div>
        </div>
      </Modal>

      {/* ── IMPORT MODAL ── */}
      <Modal open={impOpen} onClose={() => setImpOpen(false)} size="lg"
        title={t('Kvitansiya import — Excel')}
        description={t('«BFF …» roʻyxati (Қарздор ФИО + Код + Квитанция рақами) — kvitansiya mijozlarga biriktiriladi')}
        footer={impFooter}>
        <div className="space-y-4">
          <div>
            <div className="field-label">{t('Firma')} {needFirmPick && <span className="text-rose-500">*</span>}</div>
            {needFirmPick ? (
              <Select value={impFirm ? String(impFirm) : ''} label={t('Import uchun firma')} placeholder={t('Firma tanlang…')}
                options={firms!.map((f) => ({ value: String(f.id), label: f.name }))}
                onChange={(v) => { setImpFirm(v ? Number(v) : ''); resetPick(); }} />
            ) : (
              <div className="rounded-xl border border-line bg-surface-2/40 px-3 py-2 text-sm font-medium">{firmNameOf(firmId) ?? t('Barcha firmalar')}</div>
            )}
          </div>

          <div className="rounded-xl border border-line bg-surface-2/40 px-3 py-2.5 text-xs leading-relaxed text-muted">
            <div className="mb-1 font-semibold text-fg">{t('Excel ustunlari')}</div>
            <ul className="list-disc space-y-0.5 pl-4">
              <li><b>Қарздор ФИО</b> — {t('mijozni topish uchun')}</li>
              <li><b>Код</b> — {t('mijoz kodi (ixtiyoriy, aniqlik uchun)')}</li>
              <li><b>Квитанция рақами</b> — {t('biriktiriladigan kvitansiya')} <span className="text-rose-500">{t('(majburiy)')}</span></li>
              <li><i>Почта харажати</i>, <i>{t('Holat')}</i> — {t('ixtiyoriy')}</li>
            </ul>
            <a href="/konveyer/invoice-batch/import" className="mt-1.5 inline-flex items-center gap-1 font-medium text-brand-600 hover:underline dark:text-brand-400">
              <Ico.download size={12} /> {t('Namuna Excel yuklab olish')}
            </a>
          </div>
          ...
          {!done && (
            <label
              ...
              <div className="text-sm font-medium text-fg">{fileName ?? t('Excel (.xlsx) faylni tanlang yoki shu yerga tashlang')}</div>
              {gateFile ? <div className="text-[11px] font-medium text-amber-600 dark:text-amber-400">{t('Avval firmani tanlang')}</div>
                : busy && !preview ? <div className="text-[11px] text-muted">{t('Oʻqilmoqda…')}</div>
                : <div className="text-[11px] text-muted">{t('Tanlanganda avval sonlar koʻrsatiladi')}</div>}
            </label>
          )}
          ...
          {preview && !done && (
            <div className="rounded-xl border border-brand-500/30 bg-brand-500/[0.04] p-3">
              <div className="text-xs font-semibold text-fg">{t('Koʻrib chiqish — faylda')} <span className="tabular-nums">{n(preview.totalRows)}</span> {t('ta')}</div>
              <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                <Stat label={t('Biriktiriladi')} value={preview.willAssign} tone="brand" />
                <Stat label={t('Allaqachon bor')} value={preview.alreadyHas} tone="muted" />
                <Stat label={t('Arizasi topilmadi')} value={preview.notFound} tone={preview.notFound ? 'amber' : 'muted'} />
                <Stat label={t('Noaniq (F.I.O)')} value={preview.ambiguous} tone="muted" />
              </div>
              {preview.notFound > 0 && (
                <div className="mt-2.5 flex flex-wrap items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/[0.06] px-2.5 py-2 text-[11px] font-medium text-amber-700 dark:text-amber-300">
                  <span><b>{n(preview.notFound)} {t('ta')}</b> {t('mijozning arizasi topilmadi — ular saqlanmaydi.')}</span>
                  <button type="button" onClick={downloadNotFound} className="ml-auto inline-flex items-center gap-1.5 rounded-md border border-emerald-500/40 bg-emerald-500/10 px-2.5 py-1 font-semibold text-emerald-700 transition-colors hover:bg-emerald-500/15 dark:text-emerald-300">
                    <Ico.sheet size={13} /> {t('Excel qilib olish')}
                  </button>
                </div>
              )}
              {preview.willAssign > 0 && <div className="mt-2.5 text-xs font-semibold text-fg">{t('Qolgan')} {n(preview.willAssign)} {t('tasini bazaga saqlansinmi?')}</div>}
            </div>
          )}

          {done && (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-3">
              <div className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">✓ {t('Saqlandi')}</div>
              <div className="mt-1 text-xs text-muted">
                <b className="tabular-nums text-fg">{n(done.assigned)}</b> {t('biriktirildi')}{done.markedPaid ? ` · ${n(done.markedPaid)} ${t('toʻlandi')}` : ''}{done.alreadyHas ? ` · ${n(done.alreadyHas)} ${t('avval bor')}` : ''}{done.notFound ? ` · ${n(done.notFound)} ${t('arizasi topilmadi (saqlanmadi)')}` : ''}.
              </div>
            </div>
          )}
        </div>
