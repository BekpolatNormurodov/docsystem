      ...
      <button onClick={() => setOpen((v) => !v)} aria-expanded={open} className="flex w-full items-center justify-between gap-3 px-5 py-3.5 text-left">
        <div className="min-w-0">
          ...
          <div className="mt-0.5 text-[11px] tabular-nums text-muted">
            {n(rows.length)} {t('kvitansiya')} · <span className="font-medium text-emerald-600 dark:text-emerald-400">{n(paid)} {t('to‘langan')}</span> · <span className="font-medium text-amber-600 dark:text-amber-400">{n(unpaid)} {t('to‘lanmagan')}</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="whitespace-nowrap text-xs font-bold tabular-nums text-brand-700 dark:text-brand-300">{n(sum)} {t('so‘m')}</span>
          <a href={`/buxgalteriya/farmoyish?firmId=${firm.firmId}`} onClick={(e) => e.stopPropagation()} className="hidden rounded-md border border-line px-2 py-1 text-[11px] font-medium text-sky-700 hover:border-sky-500/40 dark:text-sky-300 sm:inline-flex" title={t('Farmoyish (Word)')}>Word</a>
      ...
      {open && (
        <div className="border-t border-line">
          <div className="flex flex-wrap items-center gap-x-5 gap-y-1 border-b border-line bg-surface-2/30 px-4 py-2 text-[11px] tabular-nums">
            <span className="text-emerald-600 dark:text-emerald-400">{t('To‘langan')}: <b>{n(paid)}</b> · {n(paidSum)} {t('so‘m')}</span>
            <span className="text-amber-600 dark:text-amber-400">{t('To‘lanmagan')}: <b>{n(unpaid)}</b> · {n(unpaidSum)} {t('so‘m')}</span>
            <span className="ml-auto font-semibold">{t('Jami')}: {n(sum)} {t('so‘m')}</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[36rem] text-sm">
              <thead>
                <tr className="border-b border-line bg-surface-2/40 text-[10px] uppercase tracking-wide text-muted">
                  <th className="px-4 py-2 text-left font-semibold">{t('Mijoz')}</th>
                  <th className="px-4 py-2 text-left font-semibold">{t('Kvitansiya raqami')}</th>
                  <th className="px-4 py-2 text-left font-semibold">{t('Invoice raqami')}</th>
                  <th className="px-4 py-2 text-right font-semibold">{t('Summa')}</th>
                  <th className="px-4 py-2 text-right font-semibold">{t('Holat')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={r.caseId} className="hover:bg-surface-2">
                    ...
                    <td className="px-4 py-2 text-right">
                      <button
                        ...
                        title={r.locked ? t('Keyingi bosqichga o‘tgan') : r.paid ? t('To‘langan (qaytarish uchun bosing)') : t('To‘landi deb belgilash')}
                      >
                        {busy === r.caseId ? '…' : r.paid ? t('✓ To‘langan') : t('To‘landi')}
                      </button>
                    </td>
                  </tr>
                ))}
...
export function BuxgalteriyaList({ data, snapshotId }: { data: BxData; snapshotId?: number }) {
  ...
  if (data.firms.length === 0) {
    return <div className="card grid h-40 place-items-center text-center text-sm text-muted">{t('Bu sana bo‘yicha yaratilgan invoice yo‘q.')}<br />{t('Mohigul «Invoice yaratish»da boji yaratgach shu yerda ko‘rinadi.')}</div>;
  }
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_16rem]">
      ...
      <div className="min-w-0 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <a href={sel === 'all' ? '/buxgalteriya/farmoyish?format=xlsx' : `/buxgalteriya/farmoyish?firmId=${sel}&format=xlsx`} className="inline-flex items-center gap-1.5 rounded-lg border border-sky-500/40 bg-sky-500/10 px-3 py-1.5 text-xs font-semibold text-sky-700 transition-colors hover:bg-sky-500/15 dark:text-sky-300" title={t('Farmoyish (Excel)')}>
            ...
            {t('Farmoyish (Excel)')}{sel === 'all' ? ` ${t('— hammasi (ZIP)')}` : ''}
          </a>
          ...
          {sel !== 'all' && <span className="text-[11px] text-muted">{t('(tanlangan firma bo‘yicha)')}</span>}
        </div>

        {/* Umumiy hisobot — soni va HAQIQIY summasi bilan */}
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="card p-4"><div className="text-xs text-muted">{t('Jami kvitansiya')}</div><div className="mt-0.5 text-xl font-bold tabular-nums">{n(data.total)}</div><div className="text-[11px] tabular-nums text-muted">{n(data.sum)} {t('so‘m')}</div></div>
          <div className="card p-4"><div className="text-xs text-muted">{t('To‘langan')}</div><div className="mt-0.5 text-xl font-bold tabular-nums text-emerald-600 dark:text-emerald-400">{n(data.paidCount)}</div><div className="text-[11px] tabular-nums text-emerald-600/80 dark:text-emerald-400/80">{n(data.paidSum)} {t('so‘m')}</div></div>
          <div className="card p-4"><div className="text-xs text-muted">{t('To‘lanmagan')}</div><div className="mt-0.5 text-xl font-bold tabular-nums text-amber-600 dark:text-amber-400">{n(data.unpaidCount)}</div><div className="text-[11px] tabular-nums text-amber-600/80 dark:text-amber-400/80">{n(data.unpaidSum)} {t('so‘m')}</div></div>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('Qidirish: mijoz, kvitansiya yoki invoice raqami, kod…')}
      ...
      <aside className="lg:sticky lg:top-4 lg:self-start">
        <div className="card p-2">
          <div className="px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted">{t('Firmalar')}</div>
          <ul className="space-y-0.5">
            <li>
              <button onClick={() => setSel('all')} className={`flex w-full items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 text-left transition-colors ${sel === 'all' ? 'bg-brand-500/10 text-brand-700 dark:text-brand-300' : 'hover:bg-surface-2'}`}>
                <span className="text-[13px] font-semibold">{t('Hammasi')}</span>
