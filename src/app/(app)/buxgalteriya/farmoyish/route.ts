...
export async function GET(req: NextRequest) {
  ...
  if (fid) {
    ...
    if (!res) return NextResponse.json({ error: t('Bu firmada kvitansiya yo‘q') }, { status: 404 });
  ...
  if (docs.length === 0) return NextResponse.json({ error: t('Kvitansiya yo‘q') }, { status: 404 });
