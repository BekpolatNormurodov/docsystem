...
export function AppDocDropzone({ k, label, hint, accent = 'brand', initial, info, required }: {
  ...
  return (
    <div>
      <span className="field-label flex flex-wrap items-center gap-2">{label}{required !== undefined && <ReqChip required={required} />}{info && <DocInfo kind={info} />}<span className="text-muted" title={t('Qulflangan — o‘zgartirib/o‘chirib bo‘lmaydi')}>🔒</span></span>
      {file.present ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-dashed border-line px-4 py-3.5 text-muted">
          ...
          <span className="ml-auto truncate text-[11px]">{hint} · {t('faqat kod/DB orqali')}</span>
        </div>
      )}
    </div>
  );
