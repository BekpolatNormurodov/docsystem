  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
        {fullName && <span><span className="text-muted">{t('To‘liq ism:')} </span><b>{fullName}</b></span>}
        {client.totalDebt && <span><span className="text-muted">{t('Umumiy qarz (mib):')} </span><b className="tabular-nums">{money(client.totalDebt)}</b></span>}
      </div>
      {!hasDetail && <div className="text-xs text-amber-600 dark:text-amber-300">{t('Chuqur detal (SMS) hali olinmagan — faqat ijro ishlari ro‘yxati.')}</div>}
  ...
  return (
    <div className="rounded-xl border border-line bg-surface p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className="text-sm font-semibold tabular-nums">{t('Ish')} № {c.workNumber}</span>
      ...
      <dl className="grid grid-cols-[118px_1fr] gap-x-3 gap-y-1 text-xs">
        <DRow l={t('Sud organi')} v={val(c.courtOrgan)} />
        <DRow l={t('Hujjat')} v={`${val(c.courtDocType)}${c.courtDocNumber && c.courtDocNumber !== 'Nomaʼlum' ? ' № ' + c.courtDocNumber : ''}`} />
        <DRow l={t('Hujjat sanasi')} v={val(c.courtDocDate)} />
        <DRow l={t('Kuchga kirgan')} v={val(c.courtEffectiveDate)} />
        <DRow l={t('Ijrochi')} v={val(c.executorName)} />
        <DRow l={t('Ijrochi tel')} v={val(c.executorPhone)} />
        <DRow l={t('MIB bo‘limi')} v={val(c.executorDept)} />
        <DRow l={t('MIBga kelgan')} v={val(c.mibReceivedDate)} />
        <DRow l={t('Qo‘zg‘atilgan')} v={val(c.mibInitiatedDate)} />
        <DRow l={t('Umumiy summa')} v={money(c.totalAmount)} />
        <DRow l={t('Asosiy qarz')} v={money(c.mainDebt)} />
        <DRow l={t('Ijro yig‘imi')} v={money(c.executionFee)} />
        <DRow l={t('Jarima')} v={money(c.fine)} />
        <DRow l={t('Qoldiq qarz')} v={money(c.remainingDebt)} strong />
        <DRow l={t('Bank')} v={val(c.bankName)} />
        <DRow l={t('MFO / H/r')} v={`${val(c.bankMfo)} · ${val(c.bankAccount)}`} />
      </dl>
      {c.decisions && c.decisions.length > 0 && (
        <div className="mt-2 border-t border-line pt-2 text-xs">
          <div className="mb-1 text-muted">{t('Qarorlar:')}</div>
