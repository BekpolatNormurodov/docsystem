...
export function JurnalFilters({ action, q, from, to, showUser = true }: { action: string; q: string; from: string; to: string; showUser?: boolean }) {
  ...
  const [tv, setT] = useState(to || '');

  const buildParams = (aa = a, qq = text, ff = f, tt = tv) => {
  ...
  const apply = (aa = a, qq = text, ff = f, tt = tv) => {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => apply(a, text, f, tv), 400);
  ...
  return (
    <div className="flex flex-wrap items-end gap-2">
      ...
      {showUser && (
        <div>
          <span className="mb-1 block text-xs font-medium text-muted">{t('Foydalanuvchi')}</span>
          <div className="relative">
            ...
            <input className={`${inputCls} pl-9`} value={text} onChange={(e) => setText(e.target.value)} placeholder={t('login bo‘yicha…')} />
          </div>
        </div>
      )}

      <div>
        <span className="mb-1 block text-xs font-medium text-muted">{t('Dan')}</span>
        <DatePicker value={f} onChange={(v) => { setF(v); apply(a, text, v, tv); }} max={tv || undefined} ariaLabel={t('Dan (sana)')} />
      </div>
      <div>
        <span className="mb-1 block text-xs font-medium text-muted">{t('Gacha')}</span>
        <DatePicker value={tv} onChange={(v) => { setT(v); apply(a, text, f, v); }} min={f || undefined} ariaLabel={t('Gacha (sana)')} />
      </div>
      ...
      <a
        ...
        title={t('Filtrlangan jurnalni Excelga yuklab olish')}
      >
