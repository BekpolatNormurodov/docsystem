  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        ...
        <button type="button" onClick={() => setShow((v) => !v)} aria-label={show ? t('Yashirish') : t('Ko‘rsatish')} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-muted transition-colors hover:text-fg">
...
export function PasswordForm({ username, fullName, roleLabel }: { username: string; fullName: string; roleLabel: string }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setErr(null);
    setMsg(null);
    if (next !== confirm) { setErr(t('Yangi parol tasdiqlash bilan mos emas')); return; }
    setBusy(true);
    try {
  ...
  return (
    <div className="max-w-md">
      ...
      <form onSubmit={submit} className="space-y-3.5 rounded-xl border border-line bg-surface p-5">
        <PwField label={t('Joriy parol')} value={current} onChange={setCurrent} placeholder={t('Hozirgi parolingiz')} />
        <PwField label={t('Yangi parol')} value={next} onChange={setNext} placeholder={t('Kamida 4 belgi')} />
        <PwField label={t('Yangi parolni tasdiqlang')} value={confirm} onChange={setConfirm} placeholder={t('Yangi parolni qayta kiriting')} />

        {next.length > 0 && confirm.length > 0 && next !== confirm && (
          <div className="text-xs font-medium text-amber-600 dark:text-amber-400">{t('Parollar mos emas')}</div>
        )}
