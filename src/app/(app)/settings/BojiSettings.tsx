...
export function BojiSettings() {
  ...
  return (
    <div className="card p-5">
      <div className="mb-1 text-sm font-semibold">{t('Davlat boji summasi')}</div>
      <div className="mb-4 text-xs text-muted">{t('Har bir kvitansiya (davlat boji) uchun summa. Kvitansiya hujjatida va buxgalteriya panelida ishlatiladi.')}</div>
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center gap-3 rounded-lg border border-line bg-surface px-3 py-2 max-w-xs">
            ...
            />
            <span className="text-xs text-muted">{t('soʻm')}</span>
