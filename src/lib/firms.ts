...
export interface FirmCfg {
  ...
  // xat.hippo FILIAL (branch) id override. Normally the branch is auto-derived from an existing
  // reyestr, but if that branch was later deleted/replaced in hippo the old reyestrlar keep the
  // STALE id → «Invalid targeting setup» on send. Bright's reyestrlar carry branch 12 (deleted),
  // while the org's live filial is 56 (hippo «Filiallar» → «asosiy filial», Tashkilot 3). Pin it.
  hippoBranchId?: number;
}
...
// xat.hippo FILIAL (branch) override for a firm identified by STIR. undefined → derive from a reyestr.
export const hippoBranchIdByStir = (stir: string) => firmByStir(stir)?.hippoBranchId;
