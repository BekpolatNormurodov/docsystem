...
describe('buildRestPayload', () => {
  ...
  it('shortName bo‘lmasa legalName ishlatiladi', () => {
    const p = buildRestPayload({ ...baseFirm, shortName: '' } as Firm, { amount: 20600 });
    expect(p.juridicalEntity.name).toBe('BRIGHT FUTURE FINANCING MCHJ');
  });

  it('amount majburiy — berilgan summa aynan qo‘llanadi (default yo‘q)', () => {
    expect(buildRestPayload(baseFirm, { amount: 20600 }).amount).toBe(20600);
    expect(buildRestPayload(baseFirm, { amount: 41200 }).amount).toBe(41200);
  });
});
