...
export async function collectCaseFiles(ac: any): Promise<CaseFileToUpload[]> {
  ...
  for (const doc of ac.documents || []) {
    // TALABNOMA (biz generatsiya qilган xat) sudga YUBORILMAYDI (2026-09-10) — chetlab o'tamiz.
    // Faqat yetkazilganlik + kvitansiya (TALABNOMA_RECEIPT) ketadi.
    if (String(doc.kind) === 'TALABNOMA') continue;
    try {
  ...
  try {
    ...
    try {
      ...
      //
      // Ilgari bu yerda talabnoma xati generatsiya qilinib ADOLAT'ga qo'shilardi («sud xat
      // mazmunini ko'rsin» deb). Endi OLIB TASHLANDI: sudga faqat talabnoma YETKAZILGANLIGI
      // (delivery) + KVITANSIYASI (TALABNOMA_CHECK) ketadi, biz generatsiya qilган xatning
      // O'ZI ketmaydi. Stored CaseDocument(kind=TALABNOMA) ham `collectCaseFiles`da chetlab
      // o'tiladi (upload'ga tushmaydi).
    } finally {
